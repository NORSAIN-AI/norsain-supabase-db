...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      ...
      case "get_table_data": {
        const { table, limit = 100 } = args as { table: string; limit?: number };
      ...
      case "insert_data": {
        const { table, data } = args as { table: string; data: any };
      ...
      case "update_data": {
        const { table, data, match } = args as { table: string; data: any; match: any };
      ...
      case "delete_data": {
        const { table, match } = args as { table: string; match: any };
